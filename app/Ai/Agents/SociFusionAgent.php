...
class SociFusionAgent implements Agent, Conversational, HasTools
{
    ...
    public function instructions(): Stringable|string
    {
        ...
        return str_replace('{employee_name}', $name, $template)
            ...
            ."\n2. discover_prospects — when user wants NEW / fetch / N prospects: ALWAYS pass target_count (and prefer_fresh if needed). That fetches LinkedIn with short ICP keywords (not the full product pitch), SAVES a list, returns list_hash. Never reuse engagers/old lists unless the user names that list. Without target_count: may use a strong-matching saved list"
            ."\n3. propose_strategy / draft_campaign_plan — include list_hash + list_src from discovery (auto-attached when possible). If discovery just saved a list, ALWAYS pass that list_hash — do not stage a blocked plan without it"
            ...
            ."\n- Campaign inbox AI → configure_campaign_inbox_ai (auto-reply + AI context per campaign/channel)"
